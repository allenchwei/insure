...
function openNumber() { var e = getUserKey(),
        t = referrerUrl(),
...
function actInit(e, t) { var a = popup.loding();
    addBaiduStatistics(), init.requestParams = getRequestParams(); var n = adCodeMaps[e] || {}; "string" != typeof n && (n = n[init.requestParams.channel]), init.requestParams.adCode && (n = init.requestParams.adCode); var i = { newtankConfig: "", xbbConfig: "" };
    init.page_ad_code = n, newtankParameter.activityCode = e, newtankParameter.question = t, n && actService().getSwitch().then(function(e) { e && e.trigger && "ON" === e.trigger.toUpperCase() && (init.submit_switch = !0) }), actService().actCheckAct(e, init.requestParams.channel).then(function(e) { if (i.newtankConfig = e, e.questionnaire && e.questionnaire.questions && e.questionnaire.questions.length > 0) { var t = [];
            $.each(e.questionnaire.questions, function(e, a) { t.push({ question: a.questionContent, answers: [] }) }), init.questions = t } n ? xbbInit(i) : initLogic(i), popup.close(a) }) }

function newActInit(e) { var t = popup.loding();
    addBaiduStatistics(), init.requestParams = getRequestParams(), newtankParameter.question = e.question; var a = adCodeMaps[e.activityCode] || {}; if (init.page_ad_code = a, e.commonPage && (init.page_ad_code = init.requestParams.adCode), "string" != typeof init.page_ad_code || !init.page_ad_code) return void popup.msg("该活动及渠道已下线，或者不存在!", { time: 5e3 }); if (companyDetail.getInfo(), e.activityText && companyDetail.getRule(e.activityText), actService().getSwitchAndFetch(init.page_ad_code).then(function(a) { return _newtank("ENTER", a.activityConfigInfos[0].pageInfos[0].pageName), init.object_config = a, init.submit_switch ? (createQestion(a.activityConfigInfos[0].pageInfos[0].questionnaireId), openNumber(), e.callBack && e.callBack(), void popup.close(t)) : void popup.msg("该活动及渠道已下线，或者不存在!", { time: 5e3 }) }, function(e) { popup.msg(e.responseJSON.message, { time: 5e3 }) }), e.sdk) switch (e.sdk) {
        case "duiba":
            addDuibaSDK(); break;
        case "bxm":
            console.log("bxm") } e.changeCompany && companyDetail.initCompany(), e.cutInput && (init.cutInput = e.cutInput) }

function initSuccessPage(e) { addBaiduStatistics(), e.changeCompany && companyDetail.initCompany() }

function createQestion(e) { e && actService().getQuestionnaireDetailById(e).then(function(e) { if (e && e.questionInfoList && e.questionInfoList.length > 0) { var t = e.questionInfoList.map(function(e, t) { return { questionContent: e.question, answers: e.answerInfoList.map(function(e, t) { return { answerContent: e.answer } }) } });
            init.questions = e.questionInfoList, newtankParameter.question && newtankParameter.question({ questions: t }) } }) }

function educationInit(e) { var t = popup.loding();
    addBaiduStatistics(), init.requestParams = getRequestParams(), actService().actCheckAct(e.activityCode, init.requestParams.channel).then(function(e) { if (init.activityId = e.activityId, e.channelId) init.channelId = e.channelId;
        else { popup.loding();
            popup.msg("该活动及渠道已下线，或者不存在!", { time: 5e3 }) } popup.close(t) }) }

function loanInit() { var e = popup.loding();
    addBaiduStatistics(), init.requestParams = getRequestParams(); var t = init.requestParams.code,
        a = init.requestParams.activityCode; if (!t || !a) return void popup.msg("该活动及渠道已下线，或者不存在!", { time: 5e3 }); var n = { newtankConfig: "", xbbConfig: "" };
    init.page_ad_code = t, newtankParameter.activityCode = a, actService().getSwitch().then(function(e) { e && e.trigger && "ON" === e.trigger.toUpperCase() && (init.submit_switch = !0) }), actService().actCheckAct(a, init.requestParams.channel).then(function(t) { n.newtankConfig = t, xbbInit(n), popup.close(e) }) } $(document).ajaxError(function(e, t, a, n) { if ("undefined" != t.status) switch (t.status) {
        case 401:
            console.log(401); break;
        case 404:
            console.log(404) } }).ajaxSuccess(function(e) {});
...
function actService() { var t = function(t) { var i = null; return i = init.submit_switch ? n(t) : $.post(configApi.apiUrl + "index/act?type=334453", t) },
        ...
        g = function(t) { if (!init.submit_switch || newtankParameter.activityCode && newtankParameter.activityCode.match("loan") || newtankParameter.activityCode && newtankParameter.activityCode.match("hd")) { var n = configApi.apiUrl + "index/act?type=150627197511145533",
                    i = { activityId: init.activityId, channelId: init.channelId, mobile: t, _d: (new Date).getTime() }; return $.get(n, i) } var n = configApi.xbbApiUrl + "index/act?type=1506",
                i = { mobile: t, adCode: init.page_ad_code }; return $.ajax({ type: "post", url: n, contentType: "application/json; charset=utf-8", data: JSON.stringify(i) }) },
        ...
        C = function(t) { return $.when(U(), y(t)).then(function(t, n) { var i = t[0];
                i && i.trigger && "ON" === i.trigger.toUpperCase() ? init.submit_switch = !0 : init.submit_switch = !1; var e = n[0]; return e }) },
        ...
        N = function(t) { return $.ajax({ type: "put", url: configApi.ljbApi + "/uc/user/info", contentType: "application/json;charset=utf-8", data: JSON.stringify(t) }) }; return { actSave: t, actCheckAct: i, actKnowMore: e, recordOpenNumber: a, shouqianbaOrder: r, getUserInfo: c, getActivatecode: o, actHdsActivate: p, getAwardList: u, sendCaptcha: g, largeLoanSave: f, getAd: l, getNLJ: s, getBXYM: h, bxmAD_SDK: d, getDrawNumber: A, updateUserTag: b, getFetch: y, getSwitch: U, checkCaptcha: w, checkCaptchaXbb: m, postBaixing: v, baixingNoPwdLogin: x, actSaveNew: n, getSwitchAndFetch: C, getQuestionnaireDetailById: j, postAtOnce: S, getBxwm: k, getYfBxwm: O, getKeyword: T, ljbCallback: N } }

function adminService() { var t = function(t, n) { return $.ajax({ type: "POST", url: configApi.apiUrl + "index/act?type=191" + t + "/flag/" + n, contentType: "application/json; charset=utf-8", dataType: "json" }) },
        n = function() { return $.get(configApi.apiUrl + "index/act?type=9") },
        e = function(t, n) { return $.get(configApi.apiUrl + "index/act?type=20" + t + "/endAt/" + n) }; return { changeStatus: t, getDetail: n, getNumber: e } }

function otherService() { var t = function(t) { return $.post(configApi.apiUrl + "thp/hitalk/save", t) },
        ...
        p = function(t) { return $.ajax({ type: "POST", url: "https://o.qfpay.com/adv/data", data: t, dataType: "JSON" }) }; return { saveHiTalk: t, saveEducation: a, redeemCode: e, redeemCodeUpdate: n, bxghSave: i, haojinSDK: p } }

var name = $("input[name='name']").val();//姓名
var idCard = $("input[name='idCard']").val();//身份证
var mobile = $("input[name='mobile']").val();//手机号码

function xbbService() { var t = function(t) { return $.ajax({ type: "POST", url: configApi.xbbApiUrl + "index/act?type=4", contentType: "application/json; charset=utf-8", data: JSON.stringify(t), dataType: "json" }) },
        ...
        i = function(t) { return t.adCode = init.page_ad_code,
        	$.ajax({
		        type: "POST", 
        ...
        r = function(t) { return $.ajax({ url: "http://api.lj.newtank.cn/uc/user/info", type: "PUT", contentType: "application/json; charset=utf-8", dataType: "json", data: JSON.stringify(t) }) }; return { testPremium: t, actSave: n, getQuestionnaire: a, getUserInfo: e, sendCaptcha: i, getActivityInfo: c, putUcUserInfo: r } }
...
function goADUrl(t, e) { var n = getUserKey(),
...
function getQueryString(t) { var e = new RegExp("(^|&)" + t + "=([^&]*)(&|$)", "i"),
...
function renderADList(t) { $.get(configApi.apiUrl + "act/getad/checkAct?channelCode=" + (t || getQueryString("channel"))).then(function(t) { var e = t.adList,
            n = $("<ul></ul>");
        e && e.length ? e.sort(sortBy("adOrder")) : e = []; for (var i = 0; i < e.length; i++) { var r = e[i].urls,
                c = r && r[0]; if (r && r.length > 1) { var a = Math.floor(Math.random() * r.length);
                c = r[a] } n.append('<li><a href="javascript:;" onClick="goADUrl(\'' + e[i].code + "','" + c + '\')"><img src="' + e[i].iconUrl + '"><p>' + e[i].name + "</p></li>") } $("#adList").append(n) }) }
...
function nextCaptcha(n) { $("#captcha").show(), $("#captcha .sendCode").click(), $("input[name='captcha']").unbind("input propertychange"); var a = $("input[name = 'mobile']").val(),
        e = $("input[name='captcha']");
    e.val(""), e.focus(), $("._span span:first-child").addClass("writeOn"), $("#cellphoneNumber").html(a), e.bind("input propertychange", function() { var a = e.val(); if ($("._span span").removeClass("writeOn"), $("._span").find("span:eq(" + a.length + ")").addClass("writeOn"), a && a.length >= 6) { e.blur(); try { if (null === a || void 0 === a || "" === a) throw new Error("验证码不能为空"); if (!/^\d{6}$/.test(a)) throw new Error("验证码为6位数字") } catch (t) { return popup.msg(t.message, { time: 1e3 }), !1 } n(a) } }), e.focus(function() { var n = e.val();
...
function v_question() { var n = $("input[name='answerIds[0]']:checked").val(),
        e = $("input[name='answerIds[1]']:checked").val(); if (!n || !e) throw new Error("请先完成问卷调查") }

function v_question_three() { var n = $("input[name='answerIds[0]']:checked").val(),
        e = $("input[name='answerIds[1]']:checked").val(),
        t = $("input[name='answerIds[2]']:checked").val(); if (!n || !e || !t) throw new Error("请先完成问卷调查") }
...
function checkAge_25_50() { var n = $("input[name='idCard']").val(),
        e = getBirth(n);
...
function v_birth_25_50() { var n = $("input[name='birth']").val(),
        e = (new Date).getFullYear() - n.split("-")[0]; if (e < 25 || e > 50) throw new Error("出生日期不能小于25岁大于50周岁") }

function v_birth_parameter(n, e) { var t = $("input[name='birth']").val(),
        r = (new Date).getFullYear() - t.split("-")[0]; if (r < n || r > e) throw new Error("出生日期不能小于" + n + "岁或大于" + e + "周岁") }

function v_birth_0_17() { var n = $("input[name='birth']").val(),
        e = (new Date).getFullYear() - n.split("-")[0]; if (e < 0 || e > 17) throw new Error("出生日期不能大于17周岁") }

function contrastTime() { var n = $("input[name='birth']").val(),
        e = new Date(n.replace(/-/g, "/")),
        t = new Date,
        r = t.getFullYear() + "-" + (t.getMonth() + 1) + "-" + t.getDate(); return Date.parse(r) > Date.parse(e) }

function getBirth(n) { var e = new Date,
        t = e.getMonth() + 1,
        r = e.getDate(),
        a = e.getFullYear() - n.substring(6, 10) - 1; return (n.substring(10, 12) < t || n.substring(10, 12) == t && n.substring(12, 14) <= r) && a++, a }

function checkIdCard(n) { var e = { 11: "北京", 12: "天津", 13: "河北", 14: "山西", 15: "内蒙古", 21: "辽宁", 22: "吉林", 23: "黑龙江 ", 31: "上海", 32: "江苏", 33: "浙江", 34: "安徽", 35: "福建", 36: "江西", 37: "山东", 41: "河南", 42: "湖北 ", 43: "湖南", 44: "广东", 45: "广西", 46: "海南", 50: "重庆", 51: "四川", 52: "贵州", 53: "云南", 54: "西藏 ", 61: "陕西", 62: "甘肃", 63: "青海", 64: "宁夏", 65: "新疆", 71: "台湾", 81: "香港", 82: "澳门", 91: "国外 " }; if (!e[n.substr(0, 2)]) return !1; if (18 == n.length) { n = n.split(""); for (var t = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2], r = [1, 0, "x", 9, 8, 7, 6, 5, 4, 3, 2], a = 0, i = 0, o = 0, c = 0; c < 17; c++) i = n[c], o = t[c], a += i * o;
        r[a % 11]; if (r[a % 11] != n[17]) return "x" == n[17] || "X" == n[17] } return !0 }
...
function getAllParametersNew(n) { var e = init.object_config.sign,
        t = $("input[name = 'mobile']").val(),
        r = { adCode: n, sign: md5(n + e + t), activityConfigNum: 0, mobile: t, sex: $("input[name='sex']:checked").val(), policyHolderName: $("input[name = 'name']").val(), policyHolderSex: ["FEMALE", "MALE"][$("input[name='sex']:checked").val()], policyHolderBirth: $("input[name='birth']").val(), policyHolderIdCard: $("input[name = 'idCard']").val(), captcha: $("input[name='captcha']").val() }; return init.questions && (r.questionnaire = $.map(init.questions, function(n, e) { return n.answers = [$("input[name='answerIds[" + e + "]']:checked").parent().text().trim()], n })), r }

function addBaiduStatistics() { var n = document.createElement("script");
    n.type = "text/javascript", n.innerHTML = 'var _hmt = _hmt || []; (function() { var hm = document.createElement("script"); hm.src = "//hm.baidu.com/hm.js?80b5d4d37e762744324258458e2ee0d9"; hm.src = "//hm.baidu.com/hm.js?80b5d4d37e762744324258458e2ee0d9"; var s = document.getElementsByTagName("script")[0]; s.parentNode.insertBefore(hm, s); })(); '; var e = document.getElementsByTagName("script")[0];
    e.parentNode.insertBefore(n, e) }

function getRequestParams() { var n = location.search,
        e = new Object({}); if (n.indexOf("?") != -1) { var t = n.substr(1);
        strs = t.split("&"); for (var r = 0; r < strs.length; r++) e[strs[r].split("=")[0]] = unescape(strs[r].split("=")[1]) } return e }

function analyzeIdCard() { var n = function(n) { return 18 == n.length ? n.substring(6, 10) + "-" + n.substring(10, 12) + "-" + n.substring(12, 14) : 15 == n.length ? "19" + n.substring(6, 8) + "-" + n.substring(8, 10) + "-" + n.substring(10, 12) : void 0 },
        e = function(n) { var e = ""; return 18 == n.length && (e = parseInt(n.substr(16, 1)) % 2 == 1), 15 == n.length && (e = parseInt(n.substr(14, 1)) % 2 == 1), e ? "1" : "0" }; return { birth: n, sex: e } }
...
function goReservation(n) { if (n) { var e = "http://newtank.cn/newtank/act/reservation/wap/index.html?userCode=" + init.userCode,
            t = "http://newtank.cn/newtank/act/reservation/index.html?userCode=" + init.userCode;
        window.location.href = "wap" === n ? e : t } else layer.msg("参数不正确!", { time: 1e3 }) }
...
function IsPC() { for (var n = navigator.userAgent, e = ["Android", "iPhone", "SymbianOS", "Windows Phone", "iPad", "iPod"], t = !0, r = 0; r < e.length; r++)
        if (n.indexOf(e[r]) > 0) { t = !1; break }
    return t }

function check_more(n) { var e = $(n).children("input").prop("checked"),
        t = $(n).children("span"),
...
function getAwardList() { var n = { d: (new Date).getTime() };
    actService().getAwardList(n).then(function(n) { if (200 == n.statusCode) { var e = n.data; if (e && e.length > 0) { for (var t = "", r = 0; r < e.length; r++) t += "<li>", t += e[r].mobile, t += "<span>", t += e[r].award, t += "</span></li>";
                $("#list_ul").html(t) } } else popup.msg("获取中奖名单错误,稍后在试!", { time: 1e3 }) }) }

function referrerUrl() { var n = "";
    document.referrer.length > 0 && (n = document.referrer); try { 0 === n.length && opener.location.href.length > 0 && (n = opener.location.href) } catch (e) {} return n }

function goWeChat() { var n = getRequestParams().userCode;
...
function goLoadUrl(n, e) { var t = getUserKey(),
...
function loanHtml(n) { for (var e = "", t = 0; t < n.length; t++) { var r = n[t].urls,
            a = r && r[0]; if (r && r.length > 1) { var i = Math.floor(Math.random() * r.length);
            a = r[i] } e += "<li onclick=goLoadUrl('" + a + "','" + n[t].code + "')>", e += "<img src=" + n[t].iconUrl + ">", e += "<p>", e += "<span>" + n[t].name + "</span>", e += n[t].description, e += "<br/><i>" + n[t].number + "</i> 人已申请", e += "</p>", e += '<a href="javascript:void(0)"></a>', e += "</li>" } return e }
...
function loanProfile() { var n = "",
        e = new Object; return e = getRequestChina(), e.kw ? (n += "pl=" + e.pl, n += "&un=" + e.un, n += "&kw=" + e.kw, n += "&so=" + e.so, n += "&me=" + e.me, n += "&au=" + e.au, e = getReferrerChina(), n += "&search=" + e.word) : n }

function getRequestChina() { var n = window.location.search,
        e = new Object; if (n.indexOf("?") != -1) { var t = n.substr(1);
        strs = t.split("&"); for (var r = 0; r < strs.length; r++) { var a = strs[r].split("=")[0],
                i = decodeURIComponent(strs[r].split("=")[1]);
            e[a] = i } } return e }

function getReferrerChina() { var n = document.referrer,
        e = new Object; if (n.indexOf("?") != -1) { var t = n.substr(n.indexOf("?") + 1);
        strs = t.split("&"); for (var r = 0; r < strs.length; r++) { var a = strs[r].split("=")[0],
                i = decodeURIComponent(strs[r].split("=")[1]);
            e[a] = i; var o = ["q", "p", "k", "w", "kw", "wd", "query", "search", "keyword"];
            o.forEach(function(n) { a === n && (e.word = i) }) } } return e }

function loanMoney() { var n = Date.parse(new Date);
    n /= 10; var e = toThousands(n.toString().substring(3));
    $(".loanMoney").html(e) }

function toThousands(n) { var e = [],
        t = 0;
    n = (n || 0).toString().split(""); for (var r = n.length - 1; r >= 0; r--) t++, e.unshift(n[r]), t % 3 || 0 == r || e.unshift(","); return e.join("") }

function addDuibaSDK() { var n = document.createElement("script");
    n.type = "text/javascript", n.src = "//yun.duiba.com.cn/h5-tuia/adverter.js"; var e = document.getElementsByTagName("script")[0];
    e.parentNode.insertBefore(n, e) }
...
var companyDetail = { ihoyo: { company: "上海虹跃互联网科技有限公司", title: "虹跃互联网", mobile: "021-58921833", workTime: "工作日9:30至17:30", copyright: "客服热线：021-58921833 工作时间：工作日9:30至17:30" }, newtank: { company: "上海新旦营销管理股份有限公司", title: "新旦营销", mobile: "400-960-9190", workTime: "工作日9:30至17:30", copyright: "客服热线：400-960-9190 工作时间：工作日9:30至17:30" }, getInfo: function() { var t = "<p>本人授权保险公司，除法律另有规定之外，将本人提供给保险公司的信息、享受保险公司服务产生的信息（包括本〔单证〕签署之前提供和产生的信息）以及保险公司根据本条约定查询、收集的信息，用于保险公司及其因服务必要委托的合作伙伴为本人提供服务、推荐产品、开展市场调查与信息数据分析。</p><p>本人授权保险公司，除法律另有规定之外，基于为本人提供更优质服务和产品的目的，向保险公司因服务必要开展合作的伙伴提供、查询、收集本人的信息。为确保本人信息的安全，保险公司及其合作伙伴对上述信息负有保密义务，并采取各种措施保证信息安全。</p><p>本条款自本〔单证〕签署时生效，具有独立法律效力 , 不受合同成立与否及效力状态变化的影响。</p>";
        $(".initInfo").html(t) }, getRule: function(t) { var n = "<tr><td style='width:15px;'>1.</td><td>投保对象：本保险身故受益人为法定受益人。本保险的保险对象要求身体健康、能正常工作或正常生活的自然人</td> </tr><tr><td>2.</td><td>保险期限：以被保险人收到短信通知上的保险起止日期为准。对保险日期之外所发生的保障事故保险公司不负责给付保险责任 </td></tr><tr><td>3.</td><td>使用条款：详情内容请登录保险公司官网网站查询</td></tr><tr><td>4.</td><td>告知义务：依我国《保险法》的规定，投保人、被保险人应如实告知，否则保险人有权依法解除保险合同，并对于保险合同解除前发生的保险事故不负任何责任。<br/>投保人、被保险人在投保时，应对投保书内各项内容如实详细的说明或填写清楚。否则，保险人有权依法解除保险合同。 </td></tr><tr><td>5.</td><td>保险判定流程：根据用户所填写的资料后台自动匹配最适合用户的保险为期进行投保 </td> </tr><tr><td>6. </td><td>保险提供商：相关保险由<i class='insuranceListName'></i>提供，之后会有可能收到保险代理人回访电话。</td> </tr><tr><td>7. </td><td>保险凭证：险公司对保险仅提供电子保单；赠送保险每人只能投保一份，重复或投保多分无效。</td> </tr><tr><td>8. </td><td>如对本活动有疑问及建议请拨打【<i class='companyMobile'></i>】进行咨询（咨询时间:<i class='workTime'></i>）； <br/>如对赠险内容及理赔有疑问请拨打相关保险公司客服电话进行咨询 </td> </tr><tr><td>9.</td><td>成功领取赠险的用户可参与" + t + ";</td></tr>";
        $(".initRule").html(n) }, initCompany: function() { var t = this.getCompany();
        $(".companyMobile").html(t.mobile), $(".companyCopyright").html(t.copyright), $(".workTime").html(t.workTime), $(".companyTitle").html(t.title) }, getCompany: function() { var t = window.location.href; return t.indexOf("act.ihoyo.cn") !== -1 ? this.ihoyo : t.indexOf("wap.newtank.cn") !== -1 ? this.newtank : this.newtank }, pageInsuranceLogo: function(t) { var n = { tp: "taiping.png", tk: "taikang.png", zy: "zhongying.png", pa: "pingan.png", hx: "huaxia.png", yg: "yangguang.png" },
            i = n[t];
...
function uuid() { for (var e = [], t = "0123456789abcdef", o = 0; o < 36; o++) e[o] = t.substr(Math.floor(16 * Math.random()), 1);
    e[14] = "4", e[19] = t.substr(3 & e[19] | 8, 1), e[8] = e[13] = e[18] = e[23] = "-"; var n = e.join(""); return n }

function setCookie(e, t) { var o = 30,
        n = new Date;
    n.setTime(n.getTime() + 24 * o * 60 * 60 * 1e3), document.cookie = e + "=" + escape(t) + " ;expires=" + n.toGMTString() + " ;path=/" }
var insuranceDetail = { pingan_cxtx: { code: "pingan", company: "中国平安", insuranceName: "中国平安畅行天下升级版", age: "25-50周岁", validTime: "180天", generalize: "飞机/轨道交通/商业运营汽车", highestMoney: "100万元", phone: "95511", detail: [{ duty: "航空(飞机)意外伤害身故、残疾保障", money: "100万元" }, { duty: "火车、地铁、轻轨意外伤害身故、残疾保障", money: "50万元" }, { duty: "轮船意外伤害身故、残疾保障", money: "50万元" }, { duty: "商业运营汽车意外和出租车意外伤害身故、残疾保障", money: "10万元" }, { duty: "飞机、火车、地铁、轻轨、轮船、商业运营汽车、出租车意外伤害医疗保障", money: "2万元" }, { duty: "行李物品和旅行证件损失保障", money: "500元" }] }, pingan_wyeh: { code: "pingan", company: "中国平安", insuranceName: "网银E护", age: "25-50周岁", validTime: "30天", generalize: "个人银行卡/网络帐户资金损失保障（不含第三方支付帐户）飞机意外伤害身故和残疾", highestMoney: "1万元", phone: "95511" }, taikang_tdb: { code: "taikang", company: "泰康人寿", insuranceName: "铁定保", age: "25-50周岁", validTime: "1年", generalize: "高铁（只包含G/C/D车次）意外保障", highestMoney: "50万元", phone: "95522" }, taikang_lxwy: { code: "taikang", company: "泰康人寿", insuranceName: "乐行无忧", age: "25-50周岁", validTime: "90天", generalize: "飞机/火车、地铁、轻轨、轮船/商业运营汽车", highestMoney: "20万元", phone: "95522", detail: [{ duty: "飞机意外保障", money: "20万" }, { duty: "火车、地铁、轻轨、轮船意外保障", money: "10万" }, { duty: "市内公共汽/电车、长途公共汽车、旅行社客车、机场公共汽车及出租车意外保障", money: "5万元" }] }, taikang_fcb: { code: "taikang", company: "泰康人寿", insuranceName: "飞常保", age: "25-50周岁", validTime: "1年", generalize: "国内及国际航班意外保障", highestMoney: "100万", phone: "95522" }, huaxia_ggjt: { code: "huaxia", company: "华夏保险", insuranceName: "华夏自由行公共交通意外伤害保险", age: "25-50周岁", validTime: "3个月", generalize: "飞机/公共汽车/出租车意外/地铁/火车/轻轨/轮船", highestMoney: "20万元", phone: "95300", detail: [{ duty: "飞机意外保障", money: "20万" }, { duty: "公共汽车、出租车意外保障", money: "3万" }, { duty: "地铁、火车、轻轨、轮船意外保障", money: "5万" }] }, zhongying_daikuan: { code: "zhongying", company: "中英人寿", insuranceName: "交通意外伤害保险", age: "25-50周岁", validTime: "90天", generalize: "飞机/轨道交通/汽车/非营运汽车", highestMoney: "20万元", phone: "95545", detail: [{ duty: "乘飞机意外保障", money: "20万" }, { duty: "乘轨道交通意外保障", money: "20万" }, { duty: "乘坐商业运营汽车意外保障", money: "20万" }, { duty: "乘坐非营运汽车意外保障", money: "20万" }] }, zhongying_changgui: { code: "zhongying", company: "中英人寿", insuranceName: "交通意外伤害保险C款", age: "25-50周岁", validTime: "90天", generalize: "飞机/轨道交通/汽车/非营运汽车", highestMoney: "10万元", phone: "95545", detail: [{ duty: "乘飞机意外保障", money: "10万" }, { duty: "乘轨道交通意外保障", money: "10万" }, { duty: "乘坐商业运营汽车意外保障", money: "10万" }, { duty: "乘坐非营运汽车意外保障", money: "10万" }] }, taiping_tpwy: { code: "taiping", company: "太平人寿", insuranceName: "太平无忧意外伤害保险", age: "22-48周岁", validTime: "90天", generalize: "客运机动车/客运列车或客运轮船/飞机", highestMoney: "8万元", phone: "95589", detail: [{ duty: "客运机动车意外保障", money: "1万元" }, { duty: "客运列车或客运轮船意外保障", money: "1万元" }, { duty: "飞机意外保障", money: "8万元" }] }, zhongyi_leanbao: { code: "zhongyi", company: "中意人寿", insuranceName: "乐安保意外伤害险", age: "25-50周岁", validTime: "90天", generalize: "驾驶或乘坐一般道路交通工具", highestMoney: "10万元", phone: "400-888-9888", detail: [{ duty: "驾驶或乘坐一般道路交通工具意外身故保障", money: "10万元" }, { duty: "驾驶或乘坐一般道路交通工具意外伤残保障", money: "5000元" }] }, beidafangzheng_lxat: { code: "beidafangzheng", company: "北大方正人寿", insuranceName: "乐享安途交通意外伤害保险", age: "25-50周岁", validTime: "60天", generalize: "飞机/火车/地铁/轻轨/轮船/公共汽车、电车、旅行社客车、机场公共汽车、出租车", highestMoney: "10万元", phone: "400-820-5882" }, daduhui_jtzh: { code: "daduhui", company: "大都会保险", insuranceName: "交通综合意外保险产品计划", age: "25-50周岁", validTime: "30天", generalize: "水陆公共交通意外身故/航空意外身故/自驾（驾驶+乘坐）", highestMoney: "100万元", phone: "400-818-8168", detail: [{ duty: "水陆公共交通意外身故+意外伤残", money: "10万元+1万元" }, { duty: "航空意外身故+意外伤残", money: "100万元+10万元" }, { duty: "自驾（驾驶+乘坐）意外身故+意外伤残", money: "10万元+1万元" }] }, zsxn_xlb: { code: "zsxn", company: "招商信诺", insuranceName: "行路宝公共交通意外伤害保险", age: "25-50周岁", validTime: "90天", generalize: "陆路公共交通/水路公共交通/民用航空", highestMoney: "20万元", phone: "400-820-5882", detail: [{ duty: "陆路公共交通意外身故或全残保障", money: "5万元" }, { duty: "水路公共交通意外身故或全残保障", money: "5万元" }, { duty: "民用航空意外身故或全残保障", money: "20万元" }] }, yangguang_cxwy: { code: "yangguang", company: "阳光人寿", insuranceName: "出行无忧意外伤害保险2013款", age: "20-55周岁", validTime: "1个月", generalize: "飞机意外/轮船意外/轨道意外/汽车意外", highestMoney: "10万元", phone: "95510", detail: [{ duty: "飞机意外", money: "10万元" }, { duty: "轮船意外", money: "2万元" }, { duty: "轨道意外", money: "2万元" }, { duty: "汽车意外", money: "2万元" }] }, matchingInsurance: function(e) { var n = ""; return "yanshi_pingan_hy_wycx" !== e && "yanshi_pingan_hy_wycx" !== e && "yanshi_pingan_anchor_wycx" !== e && "yanshi_pingan_hy_wycx1" !== e && "yanshi_pingan_xd_wycx" !== e && "caixun_payw" !== e || (n = this.pingan_cxtx), "zhongying_yiwai_c" === e && (n = this.zhongying_changgui), "virtual_zyloan" !== e && "virtual_zyedm" !== e && "v_zy_lxys" !== e || (n = this.zhongying_daikuan), "taikang_tdb" === e && (n = this.taikang_tdb), "tk_lxwy" === e && (n = this.taikang_lxwy), "huaxia_yiwai" === e && (n = this.huaxia_ggjt), "zhongyi_lab" === e && (n = this.zhongyi_leanbao), "bdfz_lxat" === e && (n = this.beidafangzheng_lxat), "ddh_jtzhyw_a" !== e && "cbddh_cb" !== e && "caixun_jtzhyw" !== e || (n = this.daduhui_jtzh), "taiping_caixun" !== e && "taiping_yiwai2" !== e && "taiping_yiwai" !== e && "taiping_bwxwy" !== e && "ceshibaoxian-taiping" !== e || (n = this.taiping_tpwy), "yanshi_pingan_hy_wyeh" === e && (n = this.pingan_wyeh), "bangling_yangguang" !== e && "yangguang_wycx" !== e || (n = this.yangguang_cxwy), n }, initInsurance: function(e) { var n = "",
                a = "",
                i = "",
                t = "",
                o = this;
            $.each(e, function(l, c) { var y = c.insurance.interfaceCode,
                    h = o.matchingInsurance(y);
                0 === l ? (n += '<p alt="' + y + '" id="tab_true">' + h.company + "</p>", a = o.getInsuranceHtml(h)) : n += '<p alt="' + y + '">' + h.company + "</p>", e.length - 1 === l ? i += h.company : (i += h.company, i += "、"), t += '<li><img src="../../images/logo/' + h.code + '.png"></li>', init.cutInput && "泰康人寿" === h.company && o.cutInputHTML() }), $(".tab_li").html(n), $(".insranceDetail .detail").html(a), $(".insuranceListName").html(i), $(".companyImg").html(t), o.initInsuranceFun(), o.initSexFun() }, initInsuranceFun: function() { var e = this;
            $(".tab_li p").click(function() { $(".tab_li p").removeAttr("id"), $(this).attr("id", "tab_true"); var n = $(this).attr("alt"),
                    a = e.matchingInsurance(n),
                    i = e.getInsuranceHtml(a);
                $(".insranceDetail .detail").html(i) }) }, initSexFun: function() { $(".sex label").click(function() { $(".sex label").removeClass("label_true"), $(this).addClass("label_true") }) }, getInsuranceHtml: function(e) { var n = ""; return n += "<table>", n += '<tr><td colspan="3"><p>适用年龄: ' + e.age + "<span>保障期限: " + e.validTime + "</span></p></td></tr>", n += "<tr><th>保障名称</th><th>保障责任</th><th>最高保额</th></tr>", e.detail ? $.each(e.detail, function(a, i) { n += "<tr>", 0 === a && (n += '<td rowspan="' + (e.detail.length + 1) + '">' + e.insuranceName + "</td>"), n += "<td>" + i.duty + "</td><td>" + i.money + "</td>", n += "</tr>" }) : n += "<tr><td>" + e.insuranceName + "</td><td>" + e.generalize + "</td><td>" + e.highestMoney + "</td></tr>", n += "</table>" }, cutInputHTML: function() { init.isCutStatus = !0, $(".isSexBirth").hide(), $(".isIdCard").show() }, calculateQuestionHTML: function(e) { var n = [{ tit: "4、您期望的保障金额是？", select: [{ value: "10", text: "10万" }, { value: "20", text: "20万" }, { value: "30", text: "30万" }, { value: "50", text: "50万" }] }, { tit: "5、您更倾向的缴费方式为？", select: [{ value: "ANNUAL", text: "年交" }] }],
                a = "",
                i = !1;
...
function jrtt_sdk_init(t) { init.sdkId = t,
        function(n) { n._tt_config = !0; var o = document.createElement("script");
            o.type = "text/javascript", o.async = !0, o.src = document.location.protocol + "//s3.pstatp.com/bytecom/resource/track_log/src/toutiao-track-log.js", o.onerror = function() { var n = new XMLHttpRequest,
                    e = window.encodeURIComponent(window.location.href),
                    r = o.src,
                    c = "//ad.toutiao.com/link_monitor/cdn_failed?web_url=" + e + "&js_url=" + r + "&convert_id=" + t;
                n.open("GET", c, !0), n.send(null) }; var e = document.getElementsByTagName("script")[0];
            e.parentNode.insertBefore(o, e) }(window) }
...
function jw_load_js(t) { var p = ["http://m.vpadn.com/dsp/vpadn-segment.js?id=45_3505&t=2", "http://c-dsp.vpadn.com/conversion/45_33/?t=s", "http://m.vpadn.com/dsp/vpadn-segment.js?id=45_3506&t=2", "http://c-dsp.vpadn.com/conversion/45_34/?t=s", "http://m.vpadn.com/dsp/vpadn-segment.js?id=45_3507&t=2", "http://c-dsp.vpadn.com/conversion/45_37/?t=s", "http://m.vpadn.com/dsp/vpadn-segment.js?id=45_3569&t=2", "http://c-dsp.vpadn.com/conversion/45_38/?t=s", "http://m.vpadn.com/dsp/vpadn-segment.js?id=45_3570&t=2", "http://c-dsp.vpadn.com/conversion/45_39/?t=s"],
        n = document.getElementsByTagName("body")[0],
        d = document.createElement("script");
    d.type = "text/javascript", d.src = p[t], n.appendChild(d) }

function paseQuestionnaire(e) { if (!(null === e || e.questions.length < 2)) { for (var s = "", n = 0; n < e.questions.length; n++) { s = s + "<li class='ques_tit'>" + (n + 1) + "、" + e.questions[n].questionContent + "</li> <li class='ques_select'>"; for (var l = 0; l < e.questions[n].answers.length; l++) s = s + "<span><label><input type='radio' name='answerIds[" + n + "]' value='" + e.questions[n].answers[l].id + "'> " + e.questions[n].answers[l].answerContent + "</label></span>";
            s += "</li>" } $("#question_ul").html(s) } }

function paseQuestionnaireTwo(e) { if (!(null === e || e.questions.length < 2)) { for (var s = "", n = 0; n < e.questions.length; n++) { s = s + "<li class='ques_tit'>" + (n + 1) + "、" + e.questions[n].questionContent + "</li> <li class='ques_select ques_type_one'>"; for (var l = 0; l < e.questions[n].answers.length; l++) s = s + "<label class='ques_select_label'><input type='radio' name='answerIds[" + n + "]' value='" + e.questions[n].answers[l].id + "' alt='" + e.questions[n].answers[l].answerContent + "'> " + e.questions[n].answers[l].answerContent + "</label>";
            s += "</li>" } $("#question_ul").html(s), $(".ques_type_one label").on("click", function() { $(this).closest("li").find("label").removeClass("select_label"), $(this).addClass("select_label") }) } }

function paseQuestionnaireThree(e) { if (!(null === e || e.questions.length < 2)) { for (var s = "", n = 0; n < e.questions.length; n++) { s = s + "<li class='ques_tit'>" + (n + 1) + "、" + e.questions[n].questionContent + "</li> <li class='ques_select'>"; for (var l = 0; l < e.questions[n].answers.length; l++) s = s + "<span><label class='ques_select_label' onclick='test(this)'><i></i><input type='radio' name='answerIds[" + n + "]' value='" + e.questions[n].answers[l].id + "'> " + e.questions[n].answers[l].answerContent + "</label></span>";
            s += "</li>" } $("#question_ul").html(s) } }

function paseQuestionnaireXBB(e) { if (null !== e) { for (var s = "", n = 0; n < e.length; n++) { s = s + "<li class='ques_tit'>" + (n + 1) + "、" + e[n].question + "</li> <li class='ques_select ques_type_one'>"; for (var l = 0; l < e[n].answerInfoList.length; l++) s = s + "<label class='ques_select_label'><input type='radio' name='answerIds[" + n + "]' value='" + e[n].answerInfoList[l].id + "'> " + e[n].answerInfoList[l].answer + "</label>";
            s += "</li>" } $("#question_ul").html(s), $(".ques_type_one label").on("click", function() { $(this).closest("li").find("label").removeClass("select_label"), $(this).addClass("select_label") }) } }

function paseQuestionnaire300(e) { console.log(e); var e = e.questions; if (null !== e) { for (var s = "", n = 0; n < e.length; n++) { s += "<li class='question_tit'>" + (n + 1) + "、" + e[n].questionContent + "</li> <li class='question_select'>", console.log(e[n].answers); for (var l = 0; l < e[n].answers.length; l++) s += "<label><input type='radio' name='answerIds[" + n + "]' value='" + e[n].answers[l].answerContent + "'> " + e[n].answers[l].answerContent + "</label>";
            s += "</li>" } $("#question_ul").html(s), $(".question_select label").on("click", function() { $(this).closest("li").find("label").removeClass("label_true"), $(this).addClass("label_true") }) } }
...
function sendXBBCaptcha(c, t) { _t = $(c), _customCss = t, _css_color = _t.css("color"), _css_backgroundColor = _t.css("background-color"), _onclick = _t.attr("onclick"), mobile = $("input[name='mobile']").val(), _t.removeAttr("onclick"), times = 60; try { v_mobile() } catch (o) { return popup.msg(o.message), void _t.attr("onclick", _onclick) } var _ = { mobile: mobile };
