...
class IndexController extends Controller
{
    ...
    public function index_submit()
    {
        ...
        if (!empty($get)) {
            ...
            /*身份证号格式校验*/
            if (!isCreditNo($get['idCard'])) {
                callback(202,'身份证号格式不正确!');
            }
            $idCard = $daikuanModel->where(array('USER_ID' => $get['idCard']))->find();
            Add_LOG('index', __FUNCTION__ . ' ' . __LINE__ . ' idCard：' . json_encode($idCard));
            if ($idCard) {
                callback(202,'该身份证号已经提交过申请，请勿重复提交!');
            }

            /*过滤身份证 出生年月日*/
            $date = substr($get['idCard'], 6, 8);
            $date = date('Y-m-d', strtotime($date));

            /*通过身份证号 获取性别*/
            $sex = get_xingbie($get['idCard']);
            $userAge = getUserAge($get['idCard']);
            ...
            if (empty($city)) {
                ...
                Add_LOG('index', __FUNCTION__ . ' ' . __LINE__ . ' 根据IP查询省市 ip=' . $ip_address);
                Add_LOG('index', __FUNCTION__ . ' ' . __LINE__ . ' 根据IP查询省市 result=' . json_encode($result,JSON_UNESCAPED_UNICODE));
                if ($result['ret'] == 'ok') {
                    $province = $result['data'][1];
                    $city = $result['data'][2];
                    if (empty($city)) {
                        //根据手机号码查询省市
                        $url = 'http://api.ip138.com/mobile/?mobile='.$mobile.'&oid=15997&mid=78748&sign='.md5("mobile=".$mobile."&token=4a0bc4ce0ec7fcff11b9d455d00c660f");
                        $ret = file_get_contents($url);
                        $result = json_decode($ret,true);
                        Add_LOG('index', __FUNCTION__ . ' ' . __LINE__ . ' 根据手机号码查询省市 result=' . json_encode($result,JSON_UNESCAPED_UNICODE));
                        if ($result['ret'] == 'ok') {
        	...
        	$data = array(
                ...
                "USER_NAME"     => $get['name'],//姓名
                "USER_ID"       => $get['idCard']?$get['idCard']:time(),//身份证号
                "AMOUNT"        => $get['loanAomunt'] ? $get['loanAomunt']: "0",//贷款金额
        ...
        $check = $daikuanModel->field('code,create_time,status,id,user_id,user_name')->where(array('MOBILE'=>$post['mobile']))->find();
        Add_LOG('index', __FUNCTION__ . ' ' . __LINE__ . ' check ' . json_encode($check));
        if (empty($check['user_id']) || !isCreditNo($check['user_id'])) {
            Add_LOG('index', __FUNCTION__ . ' ' . __LINE__ . ' 身份证未填或格式不正确 ');
            ajax_error('身份证未填或格式不正确，请重新注册!');
        }elseif (empty($check['user_name'])) {
            Add_LOG('index', __FUNCTION__ . ' ' . __LINE__ . ' 姓名必填 ');
            ajax_error('姓名必填，请重新注册!');
        }
        /*2：未注册成功*/
        if ($check['status'] == '2') {
    ...
    public function success()
    {
        ...
        if ($decode['status'] == "FAILED") {
            ...
            if ($zx_return['state'] == 'true') {
                ...
                $daikuan = $daikuanModel->where(array('ID' => $daikuanId))->save(array('KIND_ACCIDENT'=>$zx_return['Msg'],'POLICY_NUM'=>'','KIND_STATUS'=>2));
                Add_LOG('index', __FUNCTION__ . ' ' . __LINE__ . ' daikuan=' . $daikuan);
                Add_LOG('index', __FUNCTION__ . ' ' . __LINE__ . ' daikuanModel=' . $daikuanModel->getlastsql());
            }
            $this->display();
