...
class LuckController extends Controller
{
    public function index()
    {
        ...
        $check_channel = $channelModel->field('CHANNEL_MAP_ID')->where(array('CHANNEL_KEY'=>$channel_key,'CHANNEL_STATUS'=>1))->find();
            ...
            case 'MD5':
                ...
                foreach ($array as $value) {
                    if ($round <= $value['end'] && $round > $value['start']) {
                        ...
                        Add_LOG('index', __FUNCTION__ . ' ' . __LINE__ . ' channel_count ' . $trace);
