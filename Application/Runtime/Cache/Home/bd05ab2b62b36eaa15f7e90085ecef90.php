...
<head>
    ...
    <script>
        ...
        (function() {
          ...
          hm.src = "https://hm.baidu.com/hm.js?fd08d8b24adf528ed9e6e39fa713e348";
...
<body>
    ...
    * {
    ...
    <div class="container" style="background:url(<?php echo ($shad_back); ?>) no-repeat center center;">
        ...
        <div class="jhbox">
            <div class="reward_box">
                <img class="jhbox_img" src="/Public/bigwheel/images/jhbg.png">
                <img class="jhbox_close" src="/Public/bigwheel/images/close.png">
                <div class="jhbox_info reward_use_link" style="margin-top: -40%">
                    <a href="<?php echo ($list["link_url1"]); ?>" value="<?php echo ($list["id1"]); ?>" class="jhbox_click1" target="_blank"><img style="width: 100%;margin-left: -5%" src="<?php echo ($list["image_url1"]); ?>"></a>
                    <input type="hidden" name="jhbox_click1"  value="<?php echo ($list["id1"]); ?>">
                    <input type="hidden" name="jhbox_click2"  value="<?php echo ($list["id"]); ?>">
                    <a href="<?php echo ($list["link_url"]); ?>" value="<?php echo ($list["id"]); ?>" class="jhbox_click2" target="_blank"><img style="width: 100%;margin-left: -5%" src="<?php echo ($list["image_url"]); ?>"></a>
                </div>
            </div>
        </div>
        <div class="my_reward">
...
$(function() {
    ...
    var time_url = 'act';
    var time_data = {'CHANNEL_MAP_ID':channel,'type':'PUSH'};
    $.post(time_url, time_data, function (data){
    ...
    $('.dail_button').click(function (){
        ...
        function shade(e) {
            ...
            if (localStorage.getItem('draw_num114') <= 0) {
                ...
                url = "act";
                data = {draw_num : localStorage.getItem('draw_num114'), system : system, coupon_idlist: localStorage.getItem('coupon_idlist'),'CHANNEL_MAP_ID':channel,'type':'SHADOWE_SHOW'}
                // console.log(data)
                $.post(url, data, function (data){
                    // var e2 = $.parseJSON(data);
                    // // console.log(e2)
                    // $.each(e2,function(i,item){
                    //     // console.log(item)
                    //     $('.jhbox_info').prepend('<a class="jhbox_click'+item.id+'" value="'+item.id+'" href="'+item.link_url+'" target="_blank"><img style="width: 100%;margin-left: -5%" src="'+item.image_url+'" class="jhbox_click_src'+item.id+'"></a>');
                    // })
                    // // 修改弹出框信息
                    // $('.jhbox_img').attr('src', '/Public/bigwheel/images/jhbg.png');
                    $('.jhbox').show();
                    // is_shade = false;
                })
                is_shade = false;
            ...
            if (is_shade) {
                ...
                // 获取中奖奖券
                url = "act";
                data = {draw_num : localStorage.getItem('draw_num114'), system : system, coupon_idlist: localStorage.getItem('coupon_idlist'),'CHANNEL_MAP_ID':channel,'type':'MD5'}
                // console.log(data)
...
//                    $('.dail_annulus').css('transform', deg_annulus_str);
                    ...
                    if(e2.link == '##'){
                        ...
                        $('.reward_img').attr('src', '/Public/bigwheel/images/reward.png')
                    }
                    ...
                    $('.reward_use_link').click(function (e){
                        ...
                        var click_data = {'CHANNEL_MAP_ID':channel,'type':'LINK_CLICK','id':e2.id};
                        $.post(time_url, click_data, function (e){
    ...
    $('.jhbox_click1').click(function(){
        var id1 = $("input[name='jhbox_click1']").val();
        time_data1 = {'CHANNEL_MAP_ID':channel,'type':'SHADOWE_TIME','id':id1}
        $.post(url, time_data1, function (data){
        })
    })
    $('.jhbox_click2').click(function(){
        var id2 = $("input[name='jhbox_click2']").val();
        time_data2 = {'CHANNEL_MAP_ID':channel,'type':'SHADOWE_TIME','id':id2}
        $.post(url, time_data2, function (data){
        })
    })
